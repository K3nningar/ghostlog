<template>
    <AppLayout :title="$t('sparrows.title')">
        <div class="mx-auto max-w-7xl px-5 py-10 text-[var(--dm-text)] sm:px-8">
            ...
            <nav v-if="sparrows.links && sparrows.links.length > 3" class="mb-6 flex flex-wrap items-center justify-center gap-1" aria-label="Pagination">
                <template v-for="(link, index) in sparrows.links" :key="index">
                    <span
                        v-if="!link.url"
            ...
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
                <div
                    v-for="sparrow in sparrows.data"
                    :key="sparrow.id"
                ...
                >
                    <div class="absolute left-1 top-1 z-10 flex flex-col gap-1">
                        <span v-if="sparrow.subcategory_slug === 'secret'" class="rounded bg-purple-700 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.secret_item') }}</span>
                        <span v-if="sparrow.subcategory_slug === 'censored'" class="rounded bg-red-700 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.censored_item') }}</span>
                        <span v-if="sparrow.subcategory_slug === 'classified'" class="rounded bg-orange-700 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.classified_item') }}</span>
                        <span v-if="sparrow.subcategory_slug === 'beta'" class="rounded bg-blue-500/90 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.beta_item') }}</span>
                        <span v-if="sparrow.subcategory_slug === 'replaced'" class="rounded bg-gray-500/90 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.replaced_item') }}</span>
                    </div>
                    <img
            ...
            <p v-if="sparrows.data.length === 0" class="py-12 text-center text-white/40">{{ $t('sparrows.empty_state') }}</p>

            <nav v-if="sparrows.links && sparrows.links.length > 3" class="mt-6 flex flex-wrap items-center justify-center gap-1" aria-label="Pagination">
                <template v-for="(link, index) in sparrows.links" :key="index">
                    <span
                        v-if="!link.url"
                        class="cursor-not-allowed rounded border border-white/5 px-3 py-1 text-xs text-white/30"
...
const props = defineProps({
    sparrows: { type: Object, required: true },
...
const perPage = ref(props.filters.per_page ?? props.sparrows?.per_page ?? 48);
...
const totalCount = computed(() => props.sparrows?.meta?.total ?? props.sparrows?.total ?? props.sparrows?.data?.length ?? 0);

const availableTiers = computed(() =>
    props.tiers?.length
        ? props.tiers
        : Array.from(new Set((props.sparrows?.data ?? []).map((sparrow) => sparrow.tier_type_name).filter(Boolean)))
);
...
function openSparrowDetail(sparrow) { selectedSparrow.value = sparrow; isDrawerOpen.value = true; }
function closeSparrowDetail() { isDrawerOpen.value = false; setTimeout(() => { selectedSparrow.value = null; }, 200); }
