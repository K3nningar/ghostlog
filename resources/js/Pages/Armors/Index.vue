<template>
    <AppLayout :title="$t('armors.title')">
        <div class="mx-auto max-w-7xl px-5 py-10 text-[var(--dm-text)] sm:px-8">
            ...
            <nav v-if="armors.links && armors.links.length > 3" class="mb-6 flex flex-wrap items-center justify-center gap-1" aria-label="Pagination">
            ...
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
                <div
                    ...
                    :class="tierBorderClass(armor.tier_type)"
...
const perPage = ref(props.filters.per_page ?? props.armors?.per_page ?? 48);
