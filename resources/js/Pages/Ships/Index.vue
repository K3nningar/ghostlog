<template>
    <AppLayout :title="$t('ships.title')">
        <div class="mx-auto max-w-7xl px-5 py-10 text-[var(--dm-text)] sm:px-8">
            ...
            <nav v-if="ships.links && ships.links.length > 3" class="mb-6 flex flex-wrap items-center justify-center gap-1" aria-label="Pagination">
...
const perPage = ref(props.filters.per_page ?? props.ships?.per_page ?? 48);
