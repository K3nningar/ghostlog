<template>
    <AppLayout :title="$t('ghosts.title')">
        <div class="mx-auto max-w-7xl px-5 py-10 text-[var(--dm-text)] sm:px-8">
            <div class="mb-8 flex items-end justify-between gap-4 border-b border-white/10 pb-5">
                <div>
                    ...
                    <h1 class="text-3xl font-medium tracking-tight text-white sm:text-4xl">{{ $t('ghosts.title') }}</h1>
                </div>
            ...
            <div class="mb-6 flex flex-wrap gap-3 rounded-xl border border-white/10 bg-white/[0.03] p-4">
                <div class="min-w-[200px] flex-1">
                    ...
                    <input
                        v-model="search"
                        type="text"
                        :placeholder="$t('ghosts.search_placeholder')"
            ...
            <nav v-if="ghosts.links && ghosts.links.length > 3" class="mb-6 flex flex-wrap items-center justify-center gap-1" aria-label="Pagination">
                <template v-for="(link, index) in ghosts.links" :key="index">
                    <span
                        v-if="!link.url"
            ...
            <div class="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8">
                <div
                    ...
                    :class="tierBorderClass(ghost.tier_type)"
                    @click="openGhostDetail(ghost)"
                >
                    <div class="absolute left-1 top-1 z-10 flex flex-col gap-1">
                        <span v-if="ghost.subcategory_slug === 'secret'" class="rounded bg-purple-700 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.secret_item') }}</span>
                        <span v-if="ghost.subcategory_slug === 'censored'" class="rounded bg-red-700 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.censored_item') }}</span>
                        <span v-if="ghost.subcategory_slug === 'classified'" class="rounded bg-orange-700 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.classified_item') }}</span>
                        <span v-if="ghost.subcategory_slug === 'beta'" class="rounded bg-blue-500/90 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.beta_item') }}</span>
                        <span v-if="ghost.subcategory_slug === 'replaced'" class="rounded bg-gray-500/90 px-1.5 py-0.5 text-[10px] font-semibold text-white">{{ $t('items.replaced_item') }}</span>
                    </div>
                    <img
                        v-if="ghost.archive_icon_path"
                        :src="ghost.archive_icon_path"
                        :alt="ghost.name"
                        loading="lazy"
                    ...
                    <p class="truncate text-sm font-semibold text-white">{{ ghost.name }}</p>
                    <p class="text-xs text-white/40">{{ ghost.tier_type_name }}</p>
                </div>
            </div>

            <p v-if="ghosts.data.length === 0" class="py-12 text-center text-white/40">{{ $t('ghosts.empty_state') }}</p>

            <nav v-if="ghosts.links && ghosts.links.length > 3" class="mt-6 flex flex-wrap items-center justify-center gap-1" aria-label="Pagination">
                <template v-for="(link, index) in ghosts.links" :key="index">
                    <span
                        v-if="!link.url"
                        class="cursor-not-allowed rounded border border-white/5 px-3 py-1 text-xs text-white/30"
            ...
            <GhostDetailDrawer :ghost="selectedGhost" :open="isDrawerOpen" @close="closeGhostDetail" />
        </div>
    </AppLayout>
</template>
...
import GhostDetailDrawer from '@/Components/GhostDetailDrawer.vue';

const { t } = useI18n();

const props = defineProps({
    ghosts: { type: Object, required: true },
...
const perPage = ref(props.filters.per_page ?? props.ghosts?.per_page ?? 48);
...
const totalCount = computed(() => props.ghosts?.meta?.total ?? props.ghosts?.total ?? props.ghosts?.data?.length ?? 0);

const availableTiers = computed(() =>
    props.tiers?.length
        ? props.tiers
        : Array.from(new Set((props.ghosts?.data ?? []).map((ghost) => ghost.tier_type_name).filter(Boolean)))
);
...
function openGhostDetail(ghost) { selectedGhost.value = ghost; isDrawerOpen.value = true; }
function closeGhostDetail() { isDrawerOpen.value = false; setTimeout(() => { selectedGhost.value = null; }, 200); }
