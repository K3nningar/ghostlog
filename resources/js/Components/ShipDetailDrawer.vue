            ...
            >
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div class="flex items-center gap-5">
                        ...
                        />
                        <h2 class="text-xl font-bold text-white truncate">{{ ship.name }}</h2>
                    </div>

                    <p class="mb-1 text-xs font-semibold uppercase tracking-wide text-neutral-400">{{ ship.tier_type_name }}</p>

                    <h3 class="mb-2 text-sm font-semibold uppercase tracking-wide text-neutral-400">Description</h3>
                    <span v-if="ship.description" class="text-sm text-gray-300 italic leading-relaxed">
                        {{ ship.description }}
                    </span>

                    <div v-if="ship.hash">
                        <h3 class="mb-2 text-sm font-semibold uppercase tracking-wide text-neutral-400">Hash</h3>
                        <p class="text-sm leading-relaxed text-neutral-300">{{ ship.hash }}</p>
                    </div>
                    ...
                    >
                        {{ $t('ships.secret_item_description') }}
                    </span>
                    <span
                        v-if="ship.subcategory_slug === 'censored'"
                        class="text-[10px] bg-gray-700 text-white px-1.5 py-0.5 rounded font-semibold inline-block"
                    >
                        {{ $t('ships.censored_item_description') }}
                    </span>
                    <span
                        v-if="ship.subcategory_slug === 'classified'"
                        class="text-[10px] bg-gray-700 text-white px-1.5 py-0.5 rounded font-semibold inline-block"
                    >
...
const otherLocales = computed(() =>
    translations.value.filter((t) => t.locale !== props.ship?.locale)
);
...
watch(
    ...
    (newship) => {
        if (newship?.hash) {
            fetchTranslations(newship.hash);
        } else {
            translations.value = [];
        }
        // Ferme la modale image si le ship change
        isImageModalOpen.value = false;
    },
    { immediate: true }
);
